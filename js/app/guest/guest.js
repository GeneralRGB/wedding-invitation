...
export const guest = (() => {
    ...
    const open = (button) => {
        ...
        util.timeOut(() => confetti.openAnimation(2.5), 600);
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'Свадьба Александра и Татьяны',
            dates: '20260905T160000/20260905T230000',
            details: '16:00 - сбор гостей. 16:30 - начало церемонии.',
            location: 'Московская область, г. Красногорск, Мякининское ш., 3 (Банкетный зал Плакучая Ива)',
            ctz: config.get('tz') ?? 'Europe/Moscow',
        });

        url.search = data.toString();
        document.getElementById('button-calendar')?.addEventListener('click', () => window.open(url, '_blank'));
    };

    /**
     * Keeps the RSVP call-to-action in place until the final form URL is added.
     * @returns {void}
     */
    const buildRsvpLink = () => {
        ...
        const url = button.getAttribute('data-form-url')?.trim();
        if (url) {
