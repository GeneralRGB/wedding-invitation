...
export const comment = (() => {
    ...
    const onNullComment = () => {
        const desc = lang
            ...
            .on('ru', 'Пока пожеланий нет. Будьте первым, кто оставит теплые слова!')
            .get();
    ...
    const showOrHide = (button) => {
        ...
        button.innerText = isShow ? `Показать ответы (${ids.length})` : 'Скрыть ответы';
    ...
    const send = async (button) => {
        ...
        if (nameValue.length === 0) {
            util.notify('Введите ваше имя.').warning();
        ...
        if (!id && presence && presence.value === '0') {
            util.notify('Пожалуйста, подтвердите присутствие.').warning();
            return;
        }
        ...
        if (gifIsOpen && !gifId) {
            util.notify('Выберите GIF или вернитесь к текстовому пожеланию.').warning();
            return;
        }
        ...
        if (!gifIsOpen && form.value?.trim().length === 0) {
            util.notify('Пожелание не может быть пустым.').warning();
            return;
        }
    ...
    const cancel = async (button, id) => {
        ...
        if (gif.isOpen(id) && ((!gif.getResultId(id) && isChecklist === isPresent) || util.ask('Вы уверены?'))) {
            await gif.remove(id);
            removeInnerForm(id);
            return;
        }
        ...
        const form = document.getElementById(`form-inner-${id}`);
        if (form.value.length === 0 || (util.base64Encode(form.value) === form.getAttribute('data-original') && isChecklist === isPresent) || util.ask('Вы уверены?')) {
            removeInnerForm(id);
            return;
        }
